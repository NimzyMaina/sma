...
<div style="clear: both;"></div>	
	...
	<?php $r = 1; foreach ($rows as $row):?>
			<tr>
                ...
                <td style="width: 70px; text-align:center; vertical-align:middle;"><?php echo $this->sma->formatNumber($row->quantity); ?></td>
...
<?php if(TAX1 || TAX2 || DISCOUNT_OPTION == 1 || DISCOUNT_OPTION == 2) { echo '<tr><td colspan="'.$col.'" style="text-align:right; padding-right:10px;;">'.$this->lang->line("total").' ('. $default_currency->code.')</td><td style="text-align:right; padding-right:10px;">'.$this->ion_auth->formatMoney($inv->inv_total).'</td></tr>'; } ?>
<?php if(DISCOUNT_OPTION == 1 || DISCOUNT_OPTION == 2) { echo '<tr><td colspan="'.$col.'" style="text-align:right; padding-right:10px;;">'.$this->lang->line("discount").' ('. $default_currency->code.')</td><td style="text-align:right; padding-right:10px;">'.$this->ion_auth->formatMoney($inv->inv_discount).'</td></tr>'; } ?>
<?php if(TAX1) { echo '<tr><td colspan="'.$col.'" style="text-align:right; padding-right:10px;;">'.$this->lang->line("product_tax").' ('. $default_currency->code.')</td><td style="text-align:right; padding-right:10px;">'.$this->ion_auth->formatMoney($inv->total_tax).'</td></tr>'; } ?>
<?php if(TAX2) { echo '<tr><td colspan="'.$col.'" style="text-align:right; padding-right:10px;;">'.$this->lang->line("invoice_tax").' ('. $default_currency->code.')</td><td style="text-align:right; padding-right:10px;">'.$this->ion_auth->formatMoney($inv->total_tax2).'</td></tr>'; } ?>
<?php if($inv->shipping != 0) { echo '<tr><td colspan="'.$col.'" style="text-align:right; padding-right:10px;;">'.$this->lang->line("shipping").' ('. $default_currency->code.')</td><td style="text-align:right; padding-right:10px;">'.$this->ion_auth->formatMoney($inv->shipping).'</td></tr>'; } ?>
<tr><td colspan="<?php echo $col; ?>" style="text-align:right; padding-right:10px; font-weight:bold;"><?php echo $this->lang->line("total_amount"); ?> (<?php echo $default_currency->code; ?>)</td><td style="text-align:right; padding-right:10px; font-weight:bold;"><?php echo $this->ion_auth->formatMoney($inv->total+$inv->shipping); ?></td></tr>
...
<div id="payment_buttons">
    ...
    <br/>
                        ...
                        <input type="image" src="https://www.paypalobjects.com/en_US/i/btn/x-click-but6.gif" name="submit" alt="Pay now with PayPal">
                        <input type="hidden" name="rm" value="2">
                                                <input type="hidden" name="return" value="<?php echo site_url('module=sales&view=view_invoice&id='.$inv->id);?>"> 
                        <input type="hidden" name="cancel_return" value="<?php echo site_url('module=sales&view=view_invoice&id='.$inv->id);?>">
                        <input type="hidden" name="notify_url" value="<?php echo site_url('module=paypalipn');?>" /> 
                        <input type="hidden" name="custom" value="invoice-<?php echo $this->ion_auth->formatMoney($paypal_total); ?>">     
                        </form>
         <?php  } ?>
</div>
</body>
