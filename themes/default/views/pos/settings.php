		...
		<div class="row">
			<div class="col-lg-12">
				...
				<fieldset class="scheduler-border">
					...
					<div class="col-md-4 col-sm-4"><div class="form-group">
						...
						<?= form_input('customer', (isset($_POST['customer']) ? $_POST['customer'] : ""), 'id="customer1" data-placeholder="' . $this->lang->line("select") . ' ' . $this->lang->line("customer") . '" required="required" class="form-control" style="width:100%;"'); ?>
					</div></div>

					<div class="col-md-4 col-sm-4"><div class="form-group">
						<?= lang('display_time', 'display_time'); ?>
						<?php
						$yn = array('1' => $this->lang->line('yes'), '0' => $this->lang->line('no'));
					...
					<div class="col-md-4 col-sm-4"><div class="form-group">
						...
						<?php
						$yn = array('1' => $this->lang->line('yes'), '0' => $this->lang->line('no'));
						echo form_dropdown('rounding', $yn, $pos->rounding, 'class="form-control select" id="rounding" required="required"');
						?>
					</div></div>
					<div class="col-md-4 col-sm-4"><div class="form-group">
						<?= lang('enable_java_applet', 'enable_java_applet'); ?>
						<?php
						echo form_dropdown('enable_java_applet', $yn, $pos->java_applet, 'class="form-control select" id="enable_java_applet" required="required"');
						?>
					</div></div>
					...
					<div class="well well-sm">
						<p>Please add <strong><?=base_url()?></strong> to your java Exception Site List under Security tab.</p>
			...
			<fieldset class="scheduler-border">
				...
				if($paypal_balance) {
					...
					$blns = sizeof($paypal_balance['amount']); $r = 1;
...
<script type="text/javascript">
	$(document).ready(function (e) {
		$('#pos_setting').bootstrapValidator({feedbackIcons: {valid: 'fa fa-check', invalid: 'fa fa-times', validating: 'fa fa-refresh'}, excluded: [':disabled']});
		...
		$('#customer1').val(<?= $customer->id; ?>).select2({
			minimumInputLength: 1,
			data: [],
			initSelection: function (element, callback) {
				$.ajax({
					type: "get", async: false,
					url: "<?= site_url('customers/suggestions') ?>/" + $(element).val(),
					dataType: "json",
