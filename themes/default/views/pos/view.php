...
function product_name($name) {
    return character_limiter($name, 20);
}
if($modal) { echo '<div class="modal-dialog no-modal-header"><div class="modal-content"><div class="modal-body"><button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-2x">&times;</i></button>'; } else { ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
            ...
            <table class="table table-striped table-condensed">
                <tbody>
                    ...
                    foreach ($rows as $row) {
                        ...
                        echo '<tr><td>'.$this->sma->formatNumber($row->quantity).' x '.$this->sma->formatMoney($row->net_unit_price+($row->item_tax/$row->quantity)).'</td><td class="text-right">'.$this->sma->formatMoney($row->subtotal).'</td></tr>';
                        $r++;
                    }
                    ?>
                </tbody>
                <tfoot>
                    <tr><th><?= lang("total"); ?></th> <th class="text-right"><?= $this->sma->formatMoney($inv->total+$inv->product_tax); ?></th></tr>
                    <?php 
                /*if ($inv->product_tax != 0) { 
                    echo '<tr><th>'.lang("product_tax").'</th><th class="text-right">'.$this->sma->formatMoney($inv->product_tax).'</th></tr>';
                } */
                if ($inv->order_tax != 0) { 
                ...
                ?>
                <?php $rounding = round($this->sma->roundMoney($inv->grand_total)-$inv->grand_total, 2); ?>
                <?php if($pos_settings->rounding) { ?>
                <tr><th><?= lang("rounding"); ?></th> <th class="text-right"><?= $rounding; ?></th></tr>
                <tr><th><?= lang("grand_total"); ?></th> <th class="text-right"><?= $this->sma->formatMoney($this->sma->roundMoney($inv->grand_total-$rounding)); ?></th></tr>
                <?php } else { ?>
                <tr><th><?= lang("grand_total"); ?></th> <th class="text-right"><?= $this->sma->formatMoney($inv->grand_total); ?></th></tr>
                <?php } if($inv->paid < $inv->grand_total) { ?>
                <tr><th><?= lang("paid_amount"); ?></th> <th class="text-right"><?= $this->sma->formatMoney($inv->paid); ?></th></tr>
                <tr><th><?= lang("due_amount"); ?></th> <th class="text-right"><?= $this->sma->formatMoney($inv->grand_total-$inv->paid); ?></th></tr>
                <?php } ?>
            </tfoot>
        </table>
        <?php 
    ...
    <div class="text-center"><img src="<?=base_url()?>assets/uploads/qrcode<?=$this->session->userdata('user_id')?>.png" alt="<?=$inv->reference_no?>" /></div>
...
<?php if($modal) { echo '</div></div></div></div>'; } else { ?>
...
<?php if($pos_settings->java_applet) { 
    ...
    <script type="text/javascript">
        ...
        function printReceipt() {
            ...
            foreach ($rows as $row): ?>";
            receipt += "<?= "#" . $r ." "; ?>";
            receipt += "<?= printLine(product_name($row->product_name).($row->variant ? ' ('.$row->variant.')' : '').":".$row->tax_code, '*'); ?>" + "\n";
            ...
            <?php if ($inv->total_discount != 0) { ?>
                receipt += "<?= printLine(lang("discount") . ": " . $this->sma->formatMoney($inv->total_discount)); ?>" + "\n";
            <?php } ?>
            <?php if($pos_settings->rounding) { ?>
