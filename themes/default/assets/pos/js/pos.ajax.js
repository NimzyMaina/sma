...
$('#posdiscount').focus(function () {
	...
	$(document).on('click', '#editItem', function () {
	 	...
	 	if(parseFloat($('#pquantity').val()) != 0) {
	 		positems[item_id].row.qty = parseFloat($('#pquantity').val());
	 	}
	 	positems[item_id].row.price = parseFloat($('#pprice').val()),
	 	positems[item_id].row.tax_rate = new_pr_tax_rate,
	 	positems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : '0',
	 	positems[item_id].row.option = $('#poption').val(),
	 	positems[item_id].row.tax_method = 1,
...
/* -----------------------
	 ...
	 * Show manual item addition modal 
	 ...
	 $(document).on('click', '#addItemManually', function (e) {
	 	...
	 	positems[mid] = {"id": mid, "label": mname + ' (' + mcode + ')', "row": {"id": mid, "code": mcode, "name": mname, "quantity": mqty, "price": mprice, "tax_rate": mtax, "qty": mqty, "discount": mdiscount}, "tax_rate": mtax_rate};
	 	localStorage.setItem('positems', JSON.stringify(positems));
	 	loadItems();
	 	$('#mModal').modal('hide');
	 	$('#mcode').val('');
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		$.each(positems, function () {
			...
			//tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + item_id + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><span class="sname" id="name_' + row_no + '">' + item_name + '<br> (' + item_code + ')</span> <i class="fa fa-edit tip pointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
			...
			tr_html += '<input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_price) -item_discount + parseFloat(pr_tax_val)) + '</span></td>';
			tr_html += '<td><input class="form-control kb-pad text-center rquantity" name="quantity[]" type="text" value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
			...
			if(java_applet == 1) {
				...
				bill_data += printLine(item_qty + " x " + formatMoney(parseFloat(item_price) -item_discount + parseFloat(pr_tax_val))+": "+ formatMoney(((parseFloat(item_price) - item_discount + parseFloat(pr_tax_val)) * parseFloat(item_qty)))) + "\n";
				...
				bprTr += '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td>' + item_qty + ' x ' + formatMoney(parseFloat(item_price) -item_discount + parseFloat(pr_tax_val))+ ')</td><td style="text-align:right;">'+ formatMoney(((parseFloat(item_price) - item_discount + parseFloat(pr_tax_val)) * parseFloat(item_qty))) +'</td></tr>';
		...
		//var posBillData = {'total':total, 'product_discount':product_discount, 'order_discount':order_discount, 'product_tax':product_tax, 'invoice_tax':invoice_tax, 'count':count};
		//localStorage.setItem('posBillData', JSON.stringify(posBillData));
		...
		if(java_applet == 1) {
			bill_data += "\n"+ printLine(lang_total+': '+ formatMoney(total+product_tax-product_discount)) +"\n";
			bill_data += printLine(lang_items+': '+ (an - 1) + ' (' + (parseFloat(count) - 1) + ')') +"\n";
			if(total_discount > 0) {
				bill_data += printLine(lang_discount+': '+ formatMoney(total_discount)) +"\n";
			}
			...
			var bill_totals = '<tr><td></td><td></td></tr>';
			bill_totals += '<tr><td>'+lang_total+'</td><td style="text-align:right;">'+formatMoney(total+product_tax-product_discount)+'</td></tr>';
			bill_totals += '<tr><td>'+lang_items+'</td><td style="text-align:right;">'+(an - 1) + ' (' + (parseFloat(count) - 1) + ')</td></tr>';
			if(total_discount > 0) {
				bill_totals += '<tr><td>'+lang_discount+'</td><td style="text-align:right;">'+formatMoney(total_discount)+'</td></tr>';
			}
			if (site.settings.tax2 != 0 && invoice_tax != 0) {
...
function roundNumber(number) {
	return formatDecimal(Math.round(number * 20)/20);
}
function getNumber(x) {
	return accounting.unformat(x);
}
function formatNumber(x) {
	return accounting.formatNumber(x, site.settings.decimals, site.settings.thousands_sep == 0 ? ' ' : site.settings.thousands_sep, site.settings.decimals_sep);
}
...
function formatDecimal(x) {
	return parseFloat(x).toFixed(site.settings.decimals);
}
