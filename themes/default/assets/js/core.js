...
function formatQuantity(x) { return formatNumber(x, 2); }
...
function user_status(x) {
    var y = x.split("__");
    return y[0] == 1 ? 
    '<a href="'+site.base_url+'auth/deactivate/'+ y[1] +'" data-toggle="modal" data-target="#myModal"><span class="label label-success"><i class="fa fa-check"></i> Active</span></a>' :
    '<a href="'+site.base_url+'auth/activate/'+ y[1] +'"><span class="label label-danger"><i class="fa fa-times"></i> Inactive</span><a/>'; 
}
function row_status(x) {
    ...
    } else if(x == 'partial' || x == 'transferring') {
...
$(document).ready(function() {
    $('body').on('click', '.product_link td:not(:first-child, :nth-child(2), :last-child)', function() {
        //$('#myModal').modal({remote: site.base_url + 'products/view/' + $(this).parent('.product_link').attr('id')});
        //$('#myModal').modal('show');
        //$('#modal-loading').show();
        window.location.href = site.base_url + 'index.php/products/view/' + $(this).parent('.product_link').attr('id');
    });
    $('body').on('click', '.purchase_link td:not(:first-child :last-child)', function() {
        //$('#modal-loading').show();
        window.location.href = site.base_url + 'index.php/purchases/view/' + $(this).parent('.purchase_link').attr('id');
    });
    ...
    $('body').on('click', '.invoice_link td:not(:first-child :last-child)', function() {
        //$('#modal-loading').show();
        window.location.href = site.base_url + 'index.php/sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
    $('body').on('click', '.receipt_link td:not(:first-child :last-child)', function() {
        $('#myModal').modal({ remote: site.base_url + 'index.php/pos/view/' + $(this).parent('.receipt_link').attr('id') + '/1' });
    });
    $('body').on('click', '.return_link td', function() {
        window.location.href = site.base_url + 'index.php/sales/view_return/' + $(this).parent('.return_link').attr('id');
    });
    $('body').on('click', '.quote_link td:not(:first-child :last-child)', function() {
        //$('#modal-loading').show();
        window.location.href = site.base_url + 'index.php/quotes/view/' + $(this).parent('.quote_link').attr('id');
    });
    $('body').on('click', '.delivery_link td:not(:first-child, :last-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'index.php/sales/view_delivery/' + $(this).parent('.delivery_link').attr('id')});
        $('#myModal').modal('show');
    });
    $('body').on('click', '.customer_link td:not(:first-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'index.php/customers/edit/' + $(this).parent('.customer_link').attr('id')});
        $('#myModal').modal('show');
    });
    $('body').on('click', '.supplier_link td:not(:first-child)', function() {
        $('#myModal').modal({remote: site.base_url + 'index.php/suppliers/edit/' + $(this).parent('.supplier_link').attr('id')});
        $('#myModal').modal('show');
    });
