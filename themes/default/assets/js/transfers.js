 ...
 function loadItems() {
    ...
    if (localStorage.getItem('toitems')) {
        ...
        $.each(toitems, function () {
            ...
            //tr_html += '<td><input class="form-control text-center rcost" name="net_cost[]" type="text" value="' + item_cost + '" data-id="' + row_no + '" data-item="' + item_id + '" id="cost_' + row_no + '" onClick="this.select();"></td>';
            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + formatDecimal(item_cost) + '"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item_cost) + '</span></td>';
            ...
            if (item.options !== false) {
                $.each(item.options, function () {
                    if(this.id == item_option && item_qty > this.quantity) {
                        ...
                        if(site.settings.overselling != 1) { $('#add_transfer, #edit_transfer').attr('disabled', true); }
                    }
                });
