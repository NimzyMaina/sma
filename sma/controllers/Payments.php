...
class Payments extends MY_Controller {
                    ...
                    if(($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') &&
                            ...
                            if($inv = $this->sales_model->getInvoiceByID($invoice_no)) { 
                                ...
                                if($this->sales_model->addPayment($payment)) {
                                    ...
                                    $msg = read_file('./themes/' . $theme . '/views/email_templates/payment.html');
                                    $message = $this->parser->parse_string($msg, $parse_data);
                                    $this->sma->send_email($skrill->account_email, 'Payment has been made via Paypal', $message);
                                    $this->sma->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Paypal ('.$_POST['txn_id'].').', print_r($_POST, ture));
                                }
                            }
                    } else {

                        $this->sma->log_payment('Payment failed for Sale Reference #' . $referenc . ' via Paypal ('.$_POST['txn_id'].').', print_r($_POST, ture));

                    }
                } else if(stripos($res, "INVALID") !== false) {
                    $this->sma->log_payment('INVALID response from Paypal. Payment failed for Sale Reference #' . $referenc . ' via Paypal ('.$_POST['txn_id'].').', print_r($_POST, ture));
                }
            } //end of while
            fclose($fp);
        }
    ...
    function skrillipn() {
        ...
        if(strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['status'] == 2  && $_POST['pay_to_email'] == $skrill->account_email) {
            ...
            $this->sma->log_payment('Payment failed for Sale Reference #' . $referenc . ' via Skrill ('.$_POST['mb_transaction_id'].').', print_r($_POST, ture));
            exit;
        }
