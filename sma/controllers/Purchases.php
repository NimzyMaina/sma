...
//index or inventories page
    ...
    if($warehouse_id) {
        ...
        $this->datatables
        ->select("purchases.id as id, date, reference_no, supplier, status, grand_total, paid, (grand_total-paid) as balance, payment_status")
        ->from('purchases');
    }
...
function email($purchase_id = NULL) {
    ...
    if($this->form_validation->run() == true) {
        ...
        $attachment = $this->pdf($purchase_id, NULL, 'S'); //delete_files($attachment);
        ...
        for ($r = 0; $r < $i; $r++) {
            ...
            if(isset($item_code) && isset($item_net_cost) && isset($item_quantity)) {
                $product_details = $this->purchases_model->getProductByCode($item_code);
                    ...
                    $subtotal = (($item_net_cost * $item_quantity) + $pr_item_tax) - $pr_item_discount;
            ...
            foreach ($items as $item) {
                ...
                if($row->type == 'combo') {
                    ...
                    foreach ($combo_items as $citem) {
                        ...
                        if(!$crow) { json_decode('{}'); $crow->quantity = 0; }
                        if(isset($crow->details)) {unset($crow->details); }
                        if(isset($crow->product_details)) {unset($crow->product_details); }
...
function edit($id = NULL) {
    ...
    if($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
        ...
        foreach($inv_items as $item) {
            $row = $this->site->getProductByID($item->product_id);
            $row->expiry = (($item->expiry && $item->expiry != '0000-00-00' )? date($this->dateFormats['php_sdate'], strtotime($item->expiry)) : ''); 
            $row->qty = $item->quantity; 
                ...
                if(isset($csv_pr['code']) && isset($csv_pr['net_unit_cost']) && isset($csv_pr['quantity'])) {
                    ...
                    if($product_details = $this->purchases_model->getProductByCode($csv_pr['code'])) {
                        ...
                        if(isset($item_discount) && $this->Settings->product_discount) {
                            ...
                            if($dpos !== false) {
                                $pds = explode("%", $discount);
                                $pr_discount = $this->sma->formatDecimal((($item_net_cost*$item_quantity) * (Float)($pds[0]))/100);
                            } else {
                                $pr_discount = $this->sma->formatDecimal($discount);
                            }
                        ...
                        if(isset($item_tax_rate)) {
                            ...
                            if($tax_details->type == 1 && $tax_details->rate != 0) {
                                $item_tax = $this->sma->formatDecimal((($item_quantity * ($item_net_cost-$pr_discount)) * $tax_details->rate) / 100);
                        ...
                        $subtotal = $this->sma->formatDecimal((($item_net_cost * $item_quantity) + $item_tax) - $pr_discount);
                        $products[] = array(
                            'product_id' => $product_details->id,
                            'product_code' => $item_code,
                            'product_name' => $product_details->name,
...
function delete($id = NULL) {
    ...
    if($this->purchases_model->deletePurchase($id)) {
        echo $this->lang->line("purchase_deleted");
    }
}
