...
class Customers extends MY_Controller {
    ...
    function add() {
        ...
        if($this->form_validation->run('companies/add') == true) {
            //$u = explode(' ', $this->input->post('name'), 2);
            //$user_name = url_title($this->input->post('company'), '_');
            //$first_name = $u[0];
            //$last_name = $u[1];
    ...
    function suggestions($term = NULL, $limit = NULL) {
        $this->sma->checkPermissions('index');
        if($this->input->get('term')) { $term = $this->input->get('term', TRUE); }
        if(strlen($term) < 1) { return FALSE; }
