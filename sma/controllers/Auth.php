...
class Auth extends MY_Controller {
    ...
    function getUsers() {
        ...
        $this->datatables
        ->select("users.id as id, first_name, last_name, email, company, award_points, ".$this->db->dbprefix('groups').".name, active")
        ->from("users")
        ...
        if(!$this->Owner) { $this->datatables->unset_column('id'); }
    ...
    function login($m = NULL) {
        ...
        if($this->form_validation->run() == true) {
            /* if($_SERVER["SERVER_NAME"] != 'localhost') {
              ...
              $this->session->set_flashdata('message', lang('incorrect_captcha'));
              redirect('login');
              }
          } */
          ...
          if($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
            ...
            //$this->data['recaptcha_html'] = ($_SERVER["SERVER_NAME"] != 'localhost') ? $this->recaptcha->recaptcha_get_html() : '';
    ...
    function change_password() {
        ...
        $this->form_validation->set_rules('new_password', lang('new_password'), 'required|max_length[8]');
...
function edit_user($id = NULL) {
    ...
    if($this->form_validation->run() === TRUE && $this->ion_auth->update($user->id, $data)) {
                //$this->ion_auth->update($user->id, $data, $upg);
        if(null != $this->input->post('target') || $this->input->post('target') != 0){
        $target['u_id'] = $user->id;
        $target['target'] = $this->input->post('target');
        $target['t_date'] = date("Y-m-d h:i:s");

        $this->ion_auth->setTarget($target);
    }
...
function update_avatar($id = NULL) {
    ...
    if(!$this->ion_auth->logged_in() || !$thisOwner && $id != $this->session->userdata('user_id')) {
        $this->session->set_flashdata('warning', lang("access_denied"));
        redirect($_SERVER["HTTP_REFERER"]);
    }
...
function register() {
    ...
    if($this->form_validation->run() == true) {
        ...
        if($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data)) {
            ...
            //$this->_render_page('auth/create_user', $this->data);
            //$bc = array(array('link' => site_url('home'), 'page' => lang('home')), array('link' => '#', 'page' => lang('create_user')));
            //$meta = array('page_title' => lang('users'), 'bc' => $bc);
