	...
	function set_rack($product_id = NULL, $warehouse_id = NULL) {
		...
		if($this->form_validation->run() == true && $this->products_model->setRack($data)) {
			...
			$wh_pr = $this->products_model->getProductQuantity($product_id, $warehouse_id);
					/*if($wh_pr['quantity'] <= 0) {
						$this->session->set_flashdata('error', lang('no_qty_in_warehouse'));
						die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . site_url('products/'.$warehouse_id) . "'; }, 0);</script>");
					}*/
...
$data['quantity'] = $wh_total_quantity; 
...
function suggestions() {
	...
	if($rows) {
		foreach ($rows as $row) {
			$pr[] = array('id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'code' => $row->code, 'name' => $row->name, 'qty' => 1); 
		}
		echo json_encode($pr);
	} else {
...
$data['quantity'] = $wh_total_quantity; 
			//echo $this->sma->print_arrays($data, $warehouse_qty, $product_attributes, $photos, $items);
}
...
function import_csv() {
	...
	if($this->form_validation->run() == true) {
		...
		if(isset($_FILES["userfile"])) {
			...
			$keys = array('code', 'name', 'category_code', 'unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_code', 'variants');
		...
		$ikeys = array('code', 'name', 'category_id', 'unit', 'cost', 'price', 'alert_quantity', 'tax_rate', 'tax_method', 'subcategory_id', 'variants');

		$items = array();
		foreach (array_map(null, $pr_code, $pr_name, $pr_cat, $pr_unit, $pr_cost, $pr_price, $pr_aq, $pr_tax, $tax_method, $pr_subcat, $pr_variants) as $ikey => $value) {
...
function getdamageproducts($pdf = NULL, $xls = NULL)  {
	...
	if($pdf || $xls) {
		...
		$this->db
		->select($this->db->dbprefix('damage_products').".id as did, ".$this->db->dbprefix('damage_products').".product_id as productid, ".$this->db->dbprefix('damage_products').".date as date, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, ".$this->db->dbprefix('product_variants').".name as vname, ".$this->db->dbprefix('damage_products').".quantity as quantity, ".$this->db->dbprefix('warehouses').".name as wh");
		$this->db->from('damage_products');
		$this->db->join('products', 'products.id=damage_products.product_id', 'left');
		$this->db->join('product_variants', 'product_variants.id=damage_products.option_id', 'left');
		$this->db->join('warehouses', 'warehouses.id=damage_products.warehouse_id', 'left');
		$this->db->group_by("damage_products.id")->order_by('damage_products.date desc');
		if($product) { $this->db->like('damage_products.product_id', $product); }
		...
		if(!empty($data)) {
			...
			$this->excel->getActiveSheet()->setTitle(lang('damage_products'));
			...
			foreach ($data as $data_row) {
				...
				$this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->wh);
				$row++;
			}
		...
		$this->datatables->join('product_variants', 'product_variants.id=damage_products.option_id', 'left');
		...
		if($product) { $this->datatables->like('damage_products.product_id', $product); }
...
function add_damage($product_id = NULL, $warehouse_id = NULL) {
	...
	if($this->form_validation->run() == true) {
		...
		if(!$this->Settings->overselling){
			if($option) {
				$op_wh_qty = $this->products_model->getProductWarehouseOptionQty($option, $warehouse);
	...
	if($this->form_validation->run() == true && $this->products_model->addDamage($product_id, $date, $quantity, $warehouse, $note, $option)) {
...
function edit_damage($product_id = NULL, $id = NULL) {
	...
	if($this->form_validation->run() == true && $this->products_model->updateDamage($id, $product_id, $date, $quantity, $warehouse, $note, $option)) {
...
function view($id = NULL) {
	...
	//$this->sma->print_arrays($this->data['sold'], $this->data['purchased']);
