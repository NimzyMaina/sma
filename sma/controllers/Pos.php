...
class Pos extends MY_Controller {
    ...
    function getSales($warehouse_id = NULL) {
        ...
        $detail_link = anchor('pos/view/$1/1', '<i class="fa fa-file-text-o"></i> ' . lang('view_receipt'), 'data-toggle="modal" data-target="#myModal"');
    ...
    if($warehouse_id) {
        $this->datatables
        ->select("sales.id as id, date, reference_no, biller, customer, grand_total, paid, (grand_total-paid) as balance, payment_status, companies.email as cemail")
        ->from('sales')
...
if(!$suspend) {
    ...
    if($paid = 0) { 
        ...
        $payment_term = 'paid'; 
    } elseif($grand_total >= $paid) { $payment_term = 'partial'; }
}
    ...
    if($suspend) {
        ...
        if($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
            ...
            if(!empty($sale['message'])) {
                foreach($sale['message'] as $m) { $msg .= '<br>'.$m; }
            }
                    //echo $msg; die();
...
function close_register($user_id = NULL) {
    $this->sma->checkPermissions('index');
    if(!$this->Owner && !$this->Admin) { $user_id = $this->session->userdata('user_id'); }
...
public function write_payments_config($config) {
    ...
    $file_contents = read_file('./assets/config_dumps/payment_gateways.php');
...
function email_receipt($sale_id = NULL) {
    ...
    if(!$to) { echo json_encode(array('msg' => $this->lang->line("no_meil_provided"))); }
...
function add_payment($id = NULL) {
    ...
    if($this->form_validation->run() == true && $msg = $this->pos_model->addPayment($payment)) {
        if($msg) {
            if($msg['status'] == 0) {
                $error = '';
                foreach($msg as $m) { $error .= '<br>'.(is_array($m) ? print_r($m, true) : $m); }
