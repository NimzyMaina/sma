...
class Auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password,first_name,last_name, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, show_cost, show_price')
...
END) as percent")
    ...
    public function loginApp($identity, $password, $remember = FALSE) {
        $this->trigger_events('pre_login');
    ...
    public function setTarget($data){
        $reuslt = $this->db->insert('targets',$data);
        if ($result){
            return true;
        }
        else{
    ...
    function get_agents()     
    { 
        $this->db->select("id,CONCAT(first_name , ' ', last_name) as first_name",FALSE);
        $this->db->from('users');
        ...
        $id = array('-SELECT-');
        $fullname = array('-SELECT-');

        for ($i = 0; $i < count($result); $i++)
        {
            array_push($id, $result[$i]->id);
            array_push($fullname, $result[$i]->first_name);
        }
        return $agent = array_combine($id, $fullname);
    }
    ...
    function get_categories(){
        ...
        return $category = array_combine($id, $name);
    }
