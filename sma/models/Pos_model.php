...
class Pos_model extends CI_Model {
    ...
    public function products_count($category_id, $subcategory_id = NULL) {
        ...
        if($subcategory_id) { $this->db->where('subcategory_id', $subcategory_id); }
    ...
    public function closeRegister($rid, $user_id, $data) {
        if(!$rid) { $rid = $this->session->userdata('register_id'); }
        if(!$user_id) { $user_id = $this->session->userdata('user_id'); }
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL) {
        ...
        if($this->db->insert('sales', $data)) {
                ...
                if($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {
                    if($item['product_type'] == 'standard') {
                        ...
                        foreach($combo_items as $combo_item) {
                            ...
                            $this->addQuantity($combo_item->id, $data['warehouse_id'], $qty);
                        }
                    }
            ...
            /*if($paid == 0) {
                $this->db->update('sales', array('payment_status' => 'due'), array('id' => $sale_id));
            } elseif($paid <= ($data['grand_total']-$data['total_discount']) && $paid > 0) {
                $this->db->update('sales', array('payment_status' => 'partial'), array('id' => $sale_id));
            }*/
    ...
    public function getRegisterSales($date, $user_id = NULL) {
        if(!$date) { $date = $this->session->userdata('register_open_time'); }
        if(!$user_id) { $user_id = $this->session->userdata('user_id'); }
        $this->db->select('SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
        ->join('sales', 'sales.id=payments.sale_id', 'left')
        ->where('type', 'received')->where('payments.date >', $date);
